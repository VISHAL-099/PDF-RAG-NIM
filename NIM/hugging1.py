...
# Set Hugging Face API key (make sure you add your Hugging Face token in .env file)
os.environ["HUGGINGFACEHUB_API_TOKEN"] = "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"

# Initialize Hugging Face model pipeline (e.g., for text generation)
llm_pipeline = pipeline("text-generation", model="tiiuae/falcon-7b-instruct")

# Initialize LangChain with Hugging Face pipeline
llm = HuggingFacePipeline(pipeline=llm_pipeline)

# Streamlit app title
st.title("Hugging Face Demo: Document Retrieval and Querying")
...
def create_faiss_from_csv(csv_file):
    try:
        ...
        embeddings = HuggingFacePipeline(pipeline=llm_pipeline)  # Use HuggingFace embeddings
